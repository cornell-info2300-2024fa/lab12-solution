<header>
  <h1 id="title"><?php echo htmlspecialchars($page_title); ?> - INFO 2300</h1>

  <nav id="menu">
    <ul>
      <li class="<?php echo $nav_home_class; ?>"><a href="/">Home</a></li>
      <li class="<?php echo $nav_citations_class; ?>"><a href="/citation-policy">Citations</a></li>
      <li class="<?php echo $nav_cooking_class; ?>"><a href="/cooking-classes">Yoko's Kitchen</a></li>
      <li class="<?php echo $nav_flowershop_class; ?>"><a href="/flowershop">Flowershop</a></li>
      <li class="<?php echo $nav_transcript_class; ?>"><a href="/transcript">Transcript</a></li>
      <li class="<?php echo $nav_reviews_class; ?>"><a href="/reviews">Reviews</a></li>
      <li class="<?php echo $nav_plopbox_class; ?>"><a href="/plopbox">Plop Box</a></li>

      <?php if (is_user_logged_in()) { ?>
        <li class="float-right"><a href="<?php echo logout_url(); ?>">Sign Out</a></li>
      <?php } ?>
    </ul>
  </nav>
</header>
